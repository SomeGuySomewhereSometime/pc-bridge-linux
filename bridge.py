...
def relative(path: Path) -> str:
    return str(path.relative_to(WORKSPACE.resolve())) or "."
...
def search(req: SearchRequest):
    ...
    if platform.system() == "Windows":
        from portable import search_files
        return search_files(root, safe_path, req.query)
...
def _existing_application(policy: dict) -> int | None:
    ...
    if not executables and not prefixes:
        return None
    if platform.system() == "Windows":
        from portable import existing_application
        return existing_application(policy)
...
def process_list(req: ProcessListRequest):
    if platform.system() == "Windows":
        from portable import windows_process_list
        return windows_process_list(req)
...
def process_kill(req: ProcessKillRequest):
    if platform.system() == "Windows":
        raise HTTPException(501, "Arbitrary Windows process signalling is disabled; use app_close for owned applications")
...
def screen_capture(req: ScreenCaptureRequest):
    ...
    if platform.system() == "Windows" or os.environ.get("XDG_SESSION_TYPE") == "x11" or (os.environ.get("DISPLAY") and not os.environ.get("WAYLAND_DISPLAY") and os.environ.get("XDG_SESSION_TYPE") != "wayland"):
        from portable import capture_desktop
        backend = capture_desktop(destination, req.interactive, req.include_cursor)
        return {"ok": True, "path": relative(destination), "absolute_path": str(destination),
                "backend": backend, "bytes": destination.stat().st_size, "mime_type": "image/png"}
    errors = []
    backend = ""
