...
def checked_path(workspace: Path, config: dict, value: str, *, write: bool = False,
    ...
    # Windows temporary paths can use an 8.3 alias; compare canonical roots too.
...
def minimal_environment(*, desktop: bool = False) -> dict[str, str]:
    if sys.platform == "win32":
        keys = ("SystemRoot", "WINDIR", "COMSPEC", "TEMP", "TMP", "USERPROFILE", "LOCALAPPDATA", "APPDATA", "PATH")
        env = {k: os.environ[k] for k in keys if k in os.environ}
        env.update(HOME=str(Path.home()), USER=getpass.getuser(), PYTHONDONTWRITEBYTECODE="1")
        return env
...
def run_sandbox(workspace: Path, cwd: Path, config: dict, argv: list[str], *,
    ...
    if sys.platform != "linux":
        raise HTTPException(501, "Isolated shell, Git and patch tools require Linux Bubblewrap; unavailable on this OS")
