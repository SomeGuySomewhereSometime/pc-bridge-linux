...
os.environ.setdefault("BRIDGE_WORKSPACE", str(Path.cwd()))
...
class BridgeTests(unittest.TestCase):
    ...
    def test_process_kill_owned_allowlisted_child(self):
        ...
        try:
            # posix_spawn can return before /proc exposes the final executable.
            deadline = time.monotonic() + 3
            while bridge._process_executable(child.pid) != Path("/usr/bin/sleep").resolve():
                if child.poll() is not None or time.monotonic() >= deadline:
                    self.fail("The disposable sleep child did not finish starting")
                time.sleep(0.01)
