...
def call_bridge(func, request=None) -> Any:
    ...
    try:
        ...
        raise ToolError(f"Bridge operation unavailable or denied: {exc.detail}") from exc
...
def integration_status() -> dict:
    """Read bounded health checks for the configured local MCP integrations. Does not start/stop apps."""
    from portable import diagnostics
    return diagnostics(bridge.WORKSPACE, bridge.BRIDGE_CONFIG)
...
def screen_capture(
    ...
    """Capture Wayland via portal, X11 via MSS, or Windows via MSS; return PNG."""
    result = call_bridge(
